...
require_once 'Adapter/Creator.php';
	...
	abstract class WeFlex_Db_Model
	{
		 ...
		 */
		public function getOneByConditions( $conditions = null , $cloumns = null , $order = null ){
				...
				foreach( $conditions as $key => $value ){
					...
					if( !is_array( $value ) ){
						...
						switch( $value[0] ){
							...
							case 'like'	   :
								$keyword = $this->quote( $value[1] );
								$keyword = trim( $keyword , "'" );
								$rtn .= ' AND '.$key.' like "%'.$value[1].'%" ';
								break;	
						}
					}
				}
